...
### monthYear
A function that converts a three character string, "MAR", to the correct month number. Returns NA if value passed in is NA. Throws an error if the entered text is not valid.
```{r}
...
for ( i in 1:numArticles ){
  ...
  for ( i in 1:xmlSize(authorsResultSet) ){
    ...
    # TODO: Instead of setting affiliation to NA, I set it to UNKNOWN. Okay?
    affiliation <- "UNKNOWN"
    
    # Because not every author has an affiliation, the check for a fourth
    # element is done before extracting the info
    if ( xmlSize(author) == 4 ){
