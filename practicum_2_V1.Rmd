...
![](Practicum 2 - Star Schema.png "Practicum 2 - Star Schema")
...
## Create Table for Summary_Fact_Table
...
```{r}
# Function to find num of articles published by journal and author
...
```{r}
# Function to get num of articles with revisions based on journal and author
...
### Populate the data for the summary table (Part 2 Question 2)
...
for ( i in 2:nrow(dim_journal.df) ) {
  ...
  for ( j in 1:nrow(dim_quarter.df) ) {
    ...
    # TODO: Updated the functions and this to do all...okay?
...
## Test query on fact table to see how many articles author_id published overall

```{sql connection=dbcon}
SELECT DA.dim_author_last_name, DA.dim_author_fore_name, SFT.num_articles_published, SFT.num_articles_with_revisions
FROM Summary_Fact_Table AS SFT
JOIN DIM_Author AS DA
ON SFT.author_id = DA.dim_author_key
WHERE SFT.author_id = 3 AND SFT.dim_year = 0 AND SFT.dim_quarter = 0;
```

## Test query on fact table to see how many articles were published in a journal at any point

```{sql connection=dbcon}
SELECT DJ.dim_journal_name, SFT.num_articles_published, SFT.num_articles_with_revisions, SFT.avg_days_to_submission
FROM Summary_Fact_Table AS SFT
JOIN DIM_Journal AS DJ
ON SFT.journal_id = DJ.dim_journal_key
WHERE SFT.journal_id = 4 AND SFT.dim_year = 0 AND SFT.dim_quarter = 0;
```

## Test query to see if there is a relation between country and days_to_submission

```{sql connection=dbcon}
SELECT DJ.dim_journal_country, AVG(SFT.avg_days_to_submission)
FROM Summary_Fact_Table AS SFT
JOIN DIM_Journal AS DJ
ON SFT.journal_id = DJ.dim_journal_key
WHERE SFT.dim_year = 1 AND SFT.dim_quarter = 0 AND SFT.author_id = 0 AND SFT.avg_days_to_submission <> 0
GROUP BY DJ.dim_journal_country
ORDER BY DJ.dim_journal_country;
```
