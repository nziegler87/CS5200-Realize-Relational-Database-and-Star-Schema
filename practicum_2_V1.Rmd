...
for ( i in 1:numArticles ){
  ...
  # TODO: the [,2:ncol...] ignores the first column....because it has a PK not yet in the temp journal
  journalID <- rowExists(a.journal, journal.df[,2:ncol(journal.df)])
  ...
  # issue id
  journalIssueIDQuery <- "./MedlineCitation/Article/Journal/@IssnType"
  journalIssueID <- xpathSApply(article, journalVolumeQuery, xmlValue)
  # issn_type
  journalIssnTypeQuery <- "./MedlineCitation/Article/Journal/ISSN"
  journalIssnType <- xpathSApply(article, journalVolumeQuery, xmlValue)
