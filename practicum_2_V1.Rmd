...
# TODO: I want to save this to a table and use it for the next parts (since its a join and you can use the filters on it!)
# then we can simplify the functions with all those joins to just filter on this main table
...
```{r}
# TODO: if you're OK with it, lets use Fact_Table_Dump and COUNT(DISTINCT(ART.art_id))

# Function to find num of articles published by journal
numArticlesPublishedByJournal <- function(journal_id, year, quarter, author_id) {
...
numArticlesRevisedByJournal <- function(journal_id, year, quarter, author_id) {
...
articleIDsPublishedByJournal <- function(journal_id, year, quarter, author_id) {
  ...
  if ( quarter == 0 ){
    ...
    sqlStatement <- paste0('SELECT ART.art_id
                            ...
                            ' AND JI.publication_year = ', year, 
                            ' AND (JI.publication_month >= 7 
                             JI.publication_month <= 9)',
                            ' AND AJ.author_id = ', author_id);
    
  } else if ( quarter == 4) {
    sqlStatement <- paste0('FROM Journal AS J
                            JOIN Journal_Issue AS JI
                            ON JI.journal_id = J.j_id
                            JOIN Article AS ART
                            ON ART.journal_issue_id = JI.issue_id
  ...
  for ( j in 1:nrow(dim_quarter.df) ) {
    ...
    for ( k in 2:nrow(dim_year.df) ) {
      ...
      for ( l in 2:nrow(dim_author.df) ) {
        ...
        author_id <- dim_author.df[l,]$dim_author_key
        ...
        num_published <- numArticlesPublishedByJournal(
          journal_id, year_description, quarter, author_id)
