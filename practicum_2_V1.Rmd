...
articleIDsPublishedByJournal <- function(journal_id, year, quarter) {
  ...
  if ( quarter == 0 ){
    sqlStatement <- paste0('SELECT DISTINCT(article_id)
                            FROM Fact_Table_Dump
                            WHERE journal_id = ', journal_id, ' 
                            AND ji_pub_year = ', year);
  ...
  article_list <- dbGetQuery(dbcon, sqlStatement)
  if ( nrow(article_list) == 0 ) return(NA)
  return(article_list)
}
```
...
AND (ji_pub_month >= 6 AND ji_pub_month <= 9)
;
