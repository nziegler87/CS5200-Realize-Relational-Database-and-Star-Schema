...
```{sql connection=dbcon}
SELECT article_title, journal_name, volume, issue, publication_year, publication_month
FROM Article AS A
...
```{sql connection=dbcon}
SELECT SUBSTRING(article_title, 1, 75) AS ArticleTitle, COUNT(art_aut_id) As AuthorCount
FROM Article AS A
...
```
```{sql connection=dbcon}
SELECT SUBSTRING(article_title, 1, 50) AS ArticleTitle, journal_name,
       volume, issue, publication_year, publication_month,
       H.pub_status, H.year, H.month, H.day
FROM Article AS A
     JOIN Journal_Issue AS JI
     ON A.art_id = JI.issue_id
     JOIN Journal AS J
