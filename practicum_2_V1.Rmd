...
for ( i in 1:numArticles ){
  ...
  # # -------- #
  # 
  # # extract information for all the abstract info as three lists
  # abstractLabelQuery <- "./MedlineCitation/Article/Abstract/AbstractText/@Label"
  # abstractLabels <- xpathSApply(article, abstractLabelQuery)
  # 
  # abstractNLMQuery <- "./MedlineCitation/Article/Abstract/AbstractText/@NlmCategory"
  # abstractNLMCategories <- xpathSApply(article, abstractNLMQuery)
  # 
  # # The labels are "TEXT", so this removes the ""...a handy function
  # noquote(abstractLabels)
  # noquote(abstractNLMCategories)
  # 
  # abstractTextQuery <- "./MedlineCitation/Article/Abstract/AbstractText"
  # abstractText <- xpathSApply(article, abstractTextQuery, xmlValue)
  # 
  # # iterate through list of abstract info, create an abstract record and add it
  # # to the abstract.df
  # for ( j in 1:length(abstractLabels) ){
  #   label <- NA
  #   nlm <- NA
  #   text <- NA
  #   
  #   label <- abstractLabels[j]
  #   nlm <- abstractNLMCategories[j]
  #   text <- abstractText[j]
  #   
  #   
...
```{r}
article <- r[[1]]
  x <- "./PubmedData/History/PubMedPubDate"
  labels <- xpathSApply(article, x)
  labels[[1]]
  noquote(xmlAttrs(labels[[1]]))
