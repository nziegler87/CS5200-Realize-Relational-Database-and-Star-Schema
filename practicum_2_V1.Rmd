...
```{r}
# Function to find the article IDs associated with each journal
articleAuthors <- function(article_id) {
  
    sqlStatement <- paste0('SELECT COUNT()
                            FROM Article AS ART
                            JOIN Author AS AU
                            ON ART.article_id = AU.aut_id
...
for ( i in 2:nrow(dim_journal.df) ) {
  ...
  # loop through each quarter, including the "ALL" quarter to get yearly totals
