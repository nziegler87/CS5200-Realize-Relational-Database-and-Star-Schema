...
size <- dbGetQuery(dbcon, 'SELECT COUNT(art_id) as art_count FROM Article')
size <- size$art_count
```
...
barChart <- ggplot(data = xx, aes(fille = condition, x = years, y = incidents, fill = condition)) + geom_bar(stat = "identity", position = "dodge") + ggtitle("Trends in seasonality")

print(barChart)
```

## Trends in seasonality based on journal country
...
# adorning the bar chart
barChart <- ggplot(data = xx, aes(fille = condition, x = years, y = incidents, fill = condition)) + geom_bar(stat = "identity", position = "dodge") + ggtitle("Trends in seasonality based on journal country")
...
for ( i in 1:size ){
  ...
  pub_information[i,] <- data.frame(i, pub_date, received_date, days_difference, weeks_difference)
}
...
```{r}
# adorning the bar chart
barChart <- ggplot(data = pub_information, aes(x = art_id, y = weeks_dff))
