...
```{r}
# TODO: keep this handy: https://dtd.nlm.nih.gov/ncbi/pubmed/doc/out/180101/att-IssnType.html
...
for ( i in 1:numArticles ){
  ...
  a.journal <- data.frame(str_to_title(journalTitle), journalAbbreviation, 
  ...
  # issue id
  journalIssueIDQuery <- "./MedlineCitation/Article/Journal/ISSN"
  journalIssueID <- xpathSApply(article, journalIssueIDQuery, xmlValue)
  print(paste("Journal Issue ID:", journalIssueID))
  ...
  print(paste("Journal Type:", journalIssnType))
  ...
  an.issue <- data.frame(journalIssueID, journalID, journalIssnType,
                         journalVolume, journalIssue,
                         journalIssueYear, journalIssueMonth,
                         stringsAsFactors = FALSE)

  # If journal doesn't exist, add it to the data frame
  # TODO: Why is this column title weird
  if ( !(any(journal_issue.df[,1] == journalIssueID)) ){
    
    # Set j_id to be the next unused row num in journal.df
    newRow <- nrow(journal_issue.df) + 1
    
    # TODO: TBH, I don't understand why the first line isn't sufficient
    journal_issue.df[newRow,] <- an.issue
  }
} 
...
# Text XPath queries
```{r}
  query <- "//MedlineCitation/Article/Journal/ISSN/@IssnType"
