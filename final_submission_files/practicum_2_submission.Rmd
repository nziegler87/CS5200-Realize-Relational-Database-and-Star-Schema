...
CREATE TABLE Article (
  ...
  CONSTRAINT article_journal_issue_fk FOREIGN KEY (journal_issue_ID) REFERENCES Journal_Issue(issue_id)
);
```
...
### numArticlesPublishedByYearQuarter

Function to find num of articles published by journal.

```{r}
# Function to find num of articles published by journal
numArticlesPublishedByYearQuarter <- function(year, quarter) {
  ...
  if ( year == "ALL" & quarter == "ALL" ){
    ...
    sqlStatement <- paste0('SELECT COUNT(ART.art_id)
                            ...
                            JOIN Article AS ART
                            ON ART.journal_issue_id = JI.issue_id
                            WHERE JI.publication_year = ', year);
...
### numArticlesRevisedByYearQuarter

Function to get num of articles with revisions based on journal.

```{r}
# Function to get num of articles with revisions based on journal
numArticlesRevisedByYearQuarter <- function(year, quarter) {
...
### articleIDsPublishedByYearQuarter

Function to return a list of article IDs associated with each journal

```{r}
# Function to find the article IDs associated with each journal
articleIDsPublishedByYearQuarter <- function(year, quarter) {
...
## Add in overall summary information
...
```{r}
  ...
  for ( j in 1:nrow(dim_quarter.df) ) {
    ...
    for ( k in 1:nrow(dim_year.df) ) {
      ...
      quarter_description <- dim_quarter.df[j,]$dim_quarter_description

      # get number of articles published
      num_published <- numArticlesPublishedByYearQuarter(
        year_description, quarter_description)
      
      # retrieve the list of articles for analysis
      article_list <- articleIDsPublishedByYearQuarter(
        year_description, quarter_description)
      ...
      # get num revisions
      num_revisions <- numArticlesRevisedByYearQuarter(
        year_description, quarter_description)
  ...
  for ( j in 1:nrow(dim_quarter.df) ) {
    ...
    for ( k in 1:nrow(dim_year.df) ) {
      ...
      num_published <- numArticlesPublishedByJournal(
        journal_id, year_description, quarter_description)
  ...
  for ( j in 1:nrow(dim_quarter.df) ) {
    ...
    for ( k in 1:nrow(dim_year.df) ) {
      ...
      num_published <- numArticlesPublishedByAuthor(
        author_id, year_description, quarter_description)
...
## Confirm that fact table upload was succesfull
...
### This is then calculated using our fact table further down in the document
...
seasonal_barChart <- ggplot(
  ...
  ggtitle("Number of Articles Per Quarter") +
  guides(fill = FALSE) 
...
```{r}
# Use fact table to extract info
...
y_axis <- as.numeric(submission_by_quarter$num_articles_published)
...
## Show Tables to Confirm Database Creation
...
## Reverse Engineered ERD using MySQL Workbench

![](Reversed%20Engineered%20Schema.png "Reverse Engineered Schema")
